...
function chooseStreamSet (original, lang) {
    return original.length > 1 && lang.length > 0 ? lang : original;
}
...
module.exports = meta => command => {
    ...
    if (!audioHas2chAac) {
        const bestAudio = audio
                .reduce((best, stream) => best.channels > stream.channels ? best : stream),
