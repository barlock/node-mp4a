    ...
    async.autoInject({
        ...
        success: (inputStats, outputStats, outputProbe, next) => {
            const diffRatio = Math.abs((inputStats.size - outputStats.size) / inputStats.size),
                ...
                successfulConversion = alreadyConverted(outputProbe);

            bar.interrupt(`  ├─ Expected size ratio: ${extAcceptableChange}`);
            bar.interrupt(`  ├─ Output size ratio: ${diffRatio}`);
            bar.interrupt(`  ├─ Expected output streams found: ${successfulConversion}`);

            next(null, successfulConversion && diffRatio < extAcceptableChange)
        },
